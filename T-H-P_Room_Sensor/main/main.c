 ...
 * Task function
***********************************/
void ShowTaskFailError(BaseType_t xReturned, char* name) {
	if (xReturned == pdPASS) {
        ESP_LOGI("TAsk", "Task Created: \x1b[34m %s", name);
	} else if (xReturned == pdFAIL) {
		ESP_LOGE("TAsk", "Task Not Created [%" PRIx32 "]: \x1b[36m %s", (uint32_t)xReturned, name);
	} else if (xReturned == errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY) {
		ESP_LOGE("TAsk", "Task Not Created: Memory allocation failed: %s", name);
	} else if (xReturned == errQUEUE_BLOCKED) {
		ESP_LOGE("TAsk", "Task Not Created: Queue Blocked: %s", name);
	} else if (xReturned == errQUEUE_YIELD) {
		ESP_LOGE("TAsk", "Task Not Created: Queue Yield: %s", name);
	} else {
		ESP_LOGE("TAsk", "Task Not Created [%" PRIx32 "]: \x1b[36m %s", (uint32_t)xReturned, name);
	}
	// Add a delay to reset the WDT after the task created
...
void app_main(void)
{
    ...
    strncpy(wifi[1].ssid, "Livebox-FA70_2.4", 32);
    ...
    ShowTaskFailError(task_ret, "GPIO");
    
    task_ret = xTaskCreate(task_network, "task-network", 10 * 1024, NULL, 7, NULL);
    ShowTaskFailError(task_ret, "Wifi");
